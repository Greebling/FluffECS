...
struct MoveDetectingVector3
{
	...
	MoveDetectingVector3(int x, int y, int z)
			: x(x), y(y), z(z)
	{
	}
	...
	MoveDetectingVector3(MoveDetectingVector3 &&t) noexcept
			: x(t.x), y(t.y), z(t.z)
	{
		t.wasMoved = true;
	}
...
TEST_CASE("Object System")
{
	...
	{
		auto iFloat = float(i);
		createdEntities.push_back(myWorld.CreateEntity(Position{iFloat, iFloat * 2, iFloat * 4}, Velocity{iFloat, iFloat * 2, iFloat * 4}));
	}
	...
	struct VelocityApplySystem
	{
		...
		void operator()(Position &position, const Velocity velocity)
		{
	...
	myWorld.Foreach(velocitySystem);
	
	std::size_t counter = 0;
	myWorld.Foreach(
			[&](Position pos)
			{
				CHECK_EQ(pos.x, float(counter) *2);
				counter++;
			});
}